...
public class ArticleServiceImpl implements ArticleService {
    ...
    public ArticleResponse createArticle(ArticleResquest articleResquest) throws Exception {

        var article = mapper.toArticle(articleResquest);

        var articleSave = articleRepository.save(article);
        return mapper.toArticleResponse(articleSave);
    }

    public ArticleResponse updateArticle(String id, ArticleResquest articleResquest) {
        Article article = articleRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Article not found with id " + id));

        mergerdArticle(article, articleResquest);

        var aricleUpdate = articleRepository.save(article);
        return mapper.toArticleResponse(aricleUpdate);
    }

    private void mergerdArticle(Article article, ArticleResquest articleResquest) {
